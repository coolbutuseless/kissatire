...
static void append_snprintf (char *str, size_t n, const char *fmt, ...) {
  va_list ap;
  va_start (ap, fmt);
  const size_t len = strlen (str);
  vsnprintf (str + len, n - len - 1, fmt, ap);
  va_end (ap);
}
...
const char *kissat_log_repr (kissat *solver, unsigned lit,
  ...
  snprintf (res, FORMAT_STRING_SIZE, "%u", lit);
  if (!solver->compacting && GET_OPTION (log) > 1)
    append_snprintf (res, FORMAT_STRING_SIZE, "(%d)", kissat_export_literal (solver, lit));
  if (repr && repr[lit] != lit) {
    strcat (res, "[");
    unsigned repr_lit = repr[lit];
    append_snprintf (res, FORMAT_STRING_SIZE, "%u", repr_lit);
    if (!solver->compacting && GET_OPTION (log) > 1)
      append_snprintf (res, FORMAT_STRING_SIZE, "(%d)",
  ...
  if (!solver->compacting && GET_OPTION (log) > 1 && solver->values) {
    ...
    if (value) {
      append_snprintf (res, FORMAT_STRING_SIZE, "=%d", value);
      if (solver->assigned)
        append_snprintf (res, FORMAT_STRING_SIZE, "@%u", LEVEL (lit));
    }
  }
...
const char *kissat_log_var (kissat *solver, unsigned idx) {
  ...
  snprintf (res, FORMAT_STRING_SIZE, "variable %u (literal %s)", idx, LOGLIT (lit));
  assert (strlen (res) < FORMAT_STRING_SIZE);
  return res;
}
